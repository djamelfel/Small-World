...
public class Schtroumpf extends EspeceTer implements Herbivore {
    ...
    public Schtroumpf(boolean estLeader, boolean sexe) {
        super("Schtroumpf", 40, 70, 1, 90, 20, estLeader, false, 5, 25, Utils.getRand(3), sexe);
        getConvoiter().add("Araignee");
        getConvoiter().add("Elephant");
    }
        ...
        switch (resultat) {
            case 1:                                                                //perd
                if (getEstLeader())                                                //si leader
                    if (espece.getEstLeader() && espece instanceof Schtroumpf)        //si adversaire leader et de la meme espece
        ...
        if (getEnergie() <= 0)                                                    //si plus d'energie animal meurt
            ...
            if (getFuite() == true) {                                            //si animal en fuite
                ...
                if (getPosition().getDecors().getGraphics() == Decor.eau) {    //sinon si zone inadapter
                    ...
                    else if (getPosition().getEspece() instanceof Schtroumpf && getSexe() == false && getPosition().getEspece().getSexe() != getSexe())    //si animal meme espece de sexe different du mien et moi femelle
                    {
                    ...
                    while (finAction == false && i < vision.size()) {
                        if (vision.get(i).getEspece() != null) {                //si apperçoit animal
                            ...
                            else if (vision.get(i).getEspece() instanceof Schtroumpf) {    //sinon si animal même espece
