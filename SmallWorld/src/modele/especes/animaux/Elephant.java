...
public class Elephant extends EspeceTer implements Herbivore {


    public Elephant(boolean course, int dateNaissance, int energie, boolean estLeader, int faim, int force, boolean sexe, boolean fuite, boolean nage, int nbReproductions, int sens, boolean sommeil, int vitesse, int vitesseCourse) {
        super("Elephant", 0, 90, 5, 25, course, dateNaissance, energie, estLeader, faim, force, sexe, fuite, nage, nbReproductions, sens, sommeil, vitesse, vitesseCourse);
    }

    public Elephant(boolean estLeader, boolean sexe) {
        super("Elephant", 0, 90, 1, 70, 20, estLeader, false, 5, 25, Utils.getRand(3), sexe);
        getConvoiter().add("PoissonVolant");
        getDangeureux().add("Schtroumpf");
    }
        ...
        switch (resultat) {
            case 1:                                                                //perd
                if (getEstLeader())                                                //si leader
                    if (espece.getEstLeader() && espece instanceof Elephant)        //si adversaire leader et de la meme espece
        ...
        if (getEnergie() <= 0)                                                    //si plus d'energie animal meurt
            ...
            if (getFuite() == true) {                                            //si animal en fuite
                ...
                if (getPosition().getDecors().getGraphics() == Decor.eau) {    //sinon si zone inadapter
                    ...
                    else if (getPosition().getEspece() instanceof Elephant && getSexe() == false && getPosition().getEspece().getSexe() != getSexe())    //si animal meme espece de sexe different du mien et moi femelle
                    {
                    ...
                    while (finAction == false && i < vision.size()) {
                        if (vision.get(i).getEspece() != null) {                //si apperçoit animal
                            ...
                            else if (vision.get(i).getEspece() instanceof Elephant) {    //sinon si animal même espece
