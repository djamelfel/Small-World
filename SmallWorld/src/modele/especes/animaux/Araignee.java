...
public class Araignee extends EspeceTer implements Herbivore {
    ...
    public Araignee(boolean estLeader, boolean sexe) {
        super("Araignee", 0, 45, 1, 500, 20, estLeader, false, 5, 25, Utils.getRand(3), sexe);
		getConvoiter().add("Lion");
		...
		switch (resultat){
			case 1:																//perd
				if (getEstLeader())												//si leader 
					if (espece.getEstLeader() && espece instanceof Araignee)		//si adversaire leader et de la meme espece
...
System.out.println("combat");}
					else if (getPosition().getEspece() instanceof Araignee && getSexe() == false && getPosition().getEspece().getSexe() != getSexe() )	//si animal meme espece de sexe different du mien et moi femelle
{						
...
System.out.println("à la chasse");
							...
							else if ( vision.get(i).getEspece() instanceof Araignee ) {	//sinon si animal même espece
