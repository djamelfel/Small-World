...
public class Renard extends EspeceTer implements Herbivore {
    ...
    public Renard(boolean estLeader, boolean sexe) {
        super("Renard", 50, 100, 1, 100, 20, estLeader, false, 5, 25, Utils.getRand(3), sexe);
        ...
        switch (resultat) {
            case 1:                                                                //perd
                if (getEstLeader())                                                //si leader
                    if (espece.getEstLeader() && espece instanceof Renard)        //si adversaire leader et de la meme espece
        ...
        if (getEnergie() <= 0)                                                    //si plus d'energie animal meurt
            ...
            if (getFuite() == true) {                                            //si animal en fuite
                ...
                if (getPosition().getDecors().getGraphics() == Decor.eau) {    //sinon si zone inadapter
                    ...
                    else if (getPosition().getEspece() instanceof Renard && getSexe() == false && getPosition().getEspece().getSexe() != getSexe())    //si animal meme espece de sexe different du mien et moi femelle
                    {
                    ...
                    while (finAction == false && i < vision.size()) {
                        if (vision.get(i).getEspece() != null) {                //si apperçoit animal
                            ...
                            else if (vision.get(i).getEspece() instanceof Renard) {    //sinon si animal même espece
