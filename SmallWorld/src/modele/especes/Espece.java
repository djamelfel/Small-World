...
public class Espece {
    ...
    public void seReproduire(Espece espece) {        //pas besoin d'argument
        ...
        if (_sexe == false) {
            //Cr�ation d'un Giraffe
        }
    }

    public Boolean aFaim() {
        if (_energie < 20)    //toute espece � faim avec une energie de -20
            return true;
        return false;
    }
        ...
        if (_force < espece.getForce()) {    //perd
            ...
            if (_estLeader)                        //si leader
                if (espece.getEstLeader())            //si adversaire est leader
                    _meute.rejoindre(espece.getMeute());    //r�cupere sa meute
    ...
    public void activite() {
        if (Temps.getJeux() % 20 == 0) {        //temps � comfirmer
            setEnergie(_energie - 5);            //baisse d'�nergie � confimer
            setFaim(_faim - 20);                //baisse de faim � confirmer
        }
        if (getEnergie() <= 0)
            tuer();
        seDeplacer();
